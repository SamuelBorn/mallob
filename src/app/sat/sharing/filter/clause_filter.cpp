...
bool ClauseFilter::registerClause(const int* begin, int size) {
	...
	bool admitted = false;
	for (int i = 1; i <= 4; i++) {
		size_t h = ClauseHasher::hash(begin, size, i) % _num_bits;
...
bool ClauseFilter::likely_contains(const std::vector<int>& cls) {
    return likely_contains(cls.data(), cls.size());
}

bool ClauseFilter::likely_contains(const int* begin, int size) {
    // Block clauses above maximum length
    if (_max_clause_length > 0 && size > _max_clause_length) return false;
    ...
    for (int i = 1; i <= 4; i++) {
        size_t h = ClauseHasher::hash(begin, size, i) % _num_bits;
        // If not yet admitted, check if the bit is unset
        if (!_bitset.test(h, std::memory_order_relaxed)) return false;
    }
    return true;
}
